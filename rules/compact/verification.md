...
5. **Absence claims need boundary expansion first.** Before "X doesn't exist / no other Y / that's everything", run ALL of:
   - `git log --oneline --all` and `git branch -a` — unmerged branches hold "missing" things
   - re-run the search unfiltered and untruncated (no head/limit), with synonym vocabulary
   - check the claim's scope: cite the boundary you checked, or downgrade ("not in the checked-out tree").
