...
**A skill's own written spec is not exempt from this.** Verification gates above target code/harness *changes*; a related but distinct failure class showed up in skills — reusable prompt bundles with their own declared output format (a structured block, a named role, a file that must get saved). Skills read as "enforcement" because their instructions get injected into context in full at invoke time, but a live incident showed the same skimming failure the rest of this doc is about: a prompt-crystallization skill's entire instruction body (70k+ characters, present verbatim in context) still got invoked and then executed with all three of its required output steps skipped. The fix is the same shape as the gates above, not a longer skill file: an opt-in **contract registry** (`skill-contracts.json`) maps a skill name to its checkable output surfaces, a `PostToolUse` hook surfaces the contract into context the moment the skill is invoked (prevention), and a `Stop` hook checks the segment since the *last* invoke of that skill against the registered surfaces and bounces once, naming exactly what's missing, if any are absent (catch). Both stay silent no-ops for any skill nobody registered, and an `observe` mode lets a new contract get measured before it's trusted to block anything. See `hooks/skill-step-gate.py`, `hooks/skill-step-inject.py`, and [`rules/skill-contracts.md`](../rules/skill-contracts.md).
